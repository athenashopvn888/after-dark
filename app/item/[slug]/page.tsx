...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | After Dark Cannabis York`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://afterdarkcannabis.com/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | After Dark Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://afterdarkcannabis.com${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "Home",
        "item": "https://afterdarkcannabis.com"
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": item.category,
        "item": `https://afterdarkcannabis.com/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://afterdarkcannabis.com/item/${item.slug}`
      }
    ]
  };
}
